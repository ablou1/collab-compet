...
def ddpg(env, agent, num_agents, brain_name, n_episodes=2500, save_checkpoint=False, simu_name='test_DDPG'):
    ...
    """
    best_score = 0.5                  # Only save the agent if he gets a result better than 30.0
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        agent.end_episode()   # update the agent at the end of the episode
        ...
        print('\rEpisode {} \tScore: {:.2f} \tAverage Score: {:.2f}'.format(
            i_episode, scores.max(), np.mean(scores_window)), end="")
        if i_episode % 50 == 0:
            print('\rEpisode {} \tScore: {:.2f} \tAverage Score: {:.2f}'.format(
                i_episode, scores.max(), np.mean(scores_window)))
        ...
        if (i_episode > 100) and (np.mean(scores_window) >= best_score):
            ...
            if save_checkpoint:
                ...
                return scores_max_agent, scores_mean_last100
